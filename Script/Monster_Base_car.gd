...
@export var sound : Curve

@onready var hit_sounds = [
	preload("res://resources/hit.wav"),
	preload("res://resources/hit2.wav"),
	preload("res://resources/hit3.wav")
]

@onready var accelerationplayer = $Acceleration
@onready var brakesplayer = $brakes
@onready var hitplayer = $hit
...
func _process(delta):
	...
	soundmove(round(speed*3.8) , delta)

func calculate_dash(abc):
	var a  = last_speed - abc
	if a > 10 :
		if a > 15:
			var random_hit = hit_sounds[randi() % hit_sounds.size()]
			hitplayer.stream = random_hit
...
func all_wheel_mov_for():
	return ( FrontLeftWheel.get_rpm() > 0 and  FrontRightWheel.get_rpm() > 0 and  BackLeftWheel.get_rpm() > 0  and  BackRightWheel.get_rpm() > 0)

func all_wheel_mov_back():
	return ( FrontLeftWheel.get_rpm() < 0 and  FrontRightWheel.get_rpm() < 0 and  BackLeftWheel.get_rpm() < 0  and  BackRightWheel.get_rpm() < 0)
	
func is_on_ground():
	return (FrontLeftWheel.is_in_contact() and  FrontRightWheel.is_in_contact() and  BackLeftWheel.is_in_contact()  and  BackRightWheel.is_in_contact())

func play_brakes():
	return (all_wheel_mov_for() and Input.is_action_pressed("Throttle")) or (all_wheel_mov_back() and Input.is_action_pressed("Brakes"))

func soundmove(speed , delta):
	accelerationplayer.pitch_scale = sound.sample_baked(speed / 45)
	if Input.is_action_pressed("Throttle") :
		accelerationplayer.volume_db = lerp(accelerationplayer.volume_db , 0.0 , 1*delta )
	elif ! Input.is_action_pressed('Throttle'):
		accelerationplayer.volume_db = lerp(accelerationplayer.volume_db , -4.0 , 0.5*delta )
	if play_brakes() and is_on_ground() :
		await get_tree().create_timer(0.3).timeout
		if ! brakes and play_brakes() and is_on_ground() :
			brakes = true
			brakesplayer.playing = true
		brakesplayer.volume_db = lerp(-10 , 0 , 5 * delta)
	else:
		brakesplayer.playing = false
		brakes = false
		brakesplayer.volume_db = -10
